 ...
 *   - 答完後顯示完整對句 + 所屬章節（總敘 / 入則孝 / 出則弟 / ...）
 *   - 倒數計時 60 / 120 / 180 秒
 *   - saveScore via /api/quiz/game-score (source: 'dizigui')
 *
 * 題庫：frontend/public/data/dizigui.json (359 道；公共領域)
 */
...
export default function DiziguiGame() {
  ...
  const saveScore = useCallback(async (gained: number) => {
    ...
    try {
      await fetch('/api/quiz/game-score', {
        ...
        body: JSON.stringify({ exp: gained, reward: gained, source: 'dizigui' }),
      });
    } catch { /* silent */ }
  }, [token]);
  ...
  return (
    <div className="min-h-screen bg-teal-50 flex flex-col">
      <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <h1 className="text-xl font-bold text-teal-700">📖 弟子規</h1>
        {phase === 'playing' ? (
          <div className={`text-2xl font-black tabular-nums ${timeLeft <= 10 ? 'text-red-500' : 'text-teal-700'}`}>
            {timeLeft}s
          </div>
        ) : (
          <div className="w-16" />
      ...
      <main className="flex-1 flex flex-col items-center justify-start sm:justify-center p-4 pt-6 overflow-y-auto">
        ...
        {phase === 'idle' && !loadError && (
          <div className="card max-w-md w-full text-center">
            ...
            <h2 className="text-2xl font-black text-teal-700 mb-2">弟子規</h2>
                ...
                {DURATIONS.map((d) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl font-bold ${
                      duration === d ? 'bg-teal-500 text-white' : 'bg-white border-2 border-teal-200 text-teal-700'
                    }`}
                    type="button"
                  >
                    {d}s
          ...
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            >
              ...
              <div className="flex justify-center items-end gap-2">
                {current.phrase.split('').map((c, i) => (
                  <div key={i} className="flex flex-col items-center min-w-[2.6rem]">
                    <span className="text-teal-600 text-sm sm:text-base font-bold leading-tight bpmf-font">
                      {current.zhuyin[i] || '　'}
                    </span>
                    <span className="text-5xl sm:text-6xl font-black text-teal-900 font-serif leading-tight">
                      {c}
                    </span>
                  </div>
                ))}
              </div>
              <div className="text-center text-teal-600 text-base sm:text-lg font-bold mt-4">
                下一句是？
              </div>
              {feedback && (
                <>
                  <div className="mt-5 flex justify-center items-end gap-2 sm:gap-3">
                    ...
                    <span className="text-xl sm:text-2xl font-black text-teal-800 font-serif">
                      {current.pairContext}
                    </span>
                  </div>
                  <div className="text-center mt-2 text-sm text-gray-500">
                    出自：<span className="font-bold text-teal-700">{current.chapter}</span>
                  </div>
                </>
              )}
            </div>
                    ...
                    className={`rounded-2xl py-3 border-4 transition-colors ${
                      isCorrect
                        ...
                        : isWrongPick
                          ...
                          : 'bg-white border-teal-200 hover:bg-teal-50 active:scale-95'
                    }`}
                  >
                      ...
                      {c.phrase.split('').map((ch, ci) => (
                        <div key={ci} className="flex flex-col items-center min-w-[1.8rem]">
                          <span className={`text-[0.65rem] sm:text-xs font-bold leading-none h-3 sm:h-4 bpmf-font ${
                            isCorrect ? 'text-white/80' : 'text-teal-600'
                          }`}>
                            {c.zhuyin[ci] || '　'}
                          </span>
                          <span className={`text-2xl sm:text-3xl font-black font-serif ${
                            isCorrect ? 'text-white' : isWrongPick ? 'text-red-400' : 'text-teal-900'
                          }`}>
                            {ch}
                          </span>
                        </div>
            ...
            {feedback && (
              <button
                ...
                className="mt-4 w-full text-left bg-teal-100 border-4 border-teal-400 rounded-2xl px-5 py-4 hover:bg-teal-200 transition-colors shadow-lg"
                title="點擊跳過進入下一題"
              >
                <div className="text-base sm:text-lg text-gray-900 leading-relaxed font-medium">
                  ...
                  {' '}
                  {feedback.kind === 'wrong' && (
                    <span className="font-black text-teal-900 text-xl">{current.next}</span>
                  )}
                </div>
                {current.explanation && (
                  <div className="mt-3 text-base sm:text-lg text-gray-900 leading-relaxed">
                    <span className="font-black text-teal-700">意思：</span>
                    {current.explanation}
                  </div>
                )}
                <div className="text-sm text-teal-600 mt-2 text-right font-bold">點此立即進下一題 →</div>
              </button>
            )}
          </div>
        )}
        ...
        {phase === 'done' && (
          <div className="card max-w-md w-full text-center">
            <div className="text-5xl mb-3">🏆</div>
            <h2 className="text-2xl font-black text-teal-700 mb-4">時間到！</h2>
